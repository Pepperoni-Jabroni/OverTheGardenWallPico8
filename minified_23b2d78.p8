pico-8 cartridge // http://www.pico-8.com
version 41
__lua__
local en,n𝘲,nz,n𝘩,n6,n𝘫,n9,nk,t,nv,nj,n𝘬,nx,n𝘤,n_,np,nq,n𝘪,n7,n𝘦,n𝘴,n𝘭,n𝘮,n𝘺,n𝘣,n𝘵,n𝘢,n𝘧,n𝘳,n𝘰=split"43,110,158,185,106,190,191,202,203,205,207,222,223,238,239,240,241,242,244,245,246,247,248,249",{},(function()local e={}for n in all(split("255,candy,g#214,bird art,g#252,pumpkin shoe,g;w#253,shovel,g;w;b#254,old cat,g;k;y#126,instruments,g","#"))do local n=split(n)add(e,{spridx=n[1],name=n[2],charids=split(n[3],";")})end return e end)(),false,nil,nil,{},0,0,nil,nil,false,{},"boot",1,nil,{},{},{},{},{},0,0,0,0,0,{},{}local n𝘣,n𝘯,n𝘱,n𝘨,n𝘶,ee,n𝘷={function()return h"woods1,right_of,8"end,function()n"5"end,function()return 𝘪(1,"woods1")and(nk~=8or t~=8)end,function()n"6"end,function()return 𝘪(1,"woods1",13,7)end,function()n"18"end,function()return 𝘪(1,"woods2")end,function()end,function()return m(5,7,"woods2")end,function()n"3"𝘱()end,function()return b"woods2,m"end,function()n"4"end,function()return d"4"end,function()w"m,15|0|7|7"end,function()return#n7==2and h"woods3,below,13"end,function()n"7"end,function()return m(7,10,"woods3")end,function()n"8"end,function()return d"8"end,function()add(n7,{charid="b",x=7,y=10,mapid=np})end,function()return b"millandriver,m"end,function()n"9"end,function()return d"9"end,function()w"m,7|3|7|4"end,function()return np=="mill"and#n7==2end,function()n6=nil q{"k"}local e=l"k"e.mapid,e.x,e.y="millandriver",11,6o"millandriver".discvrdtiles={}g("?","millandriver",12,6)n"16"add(n9,{spridx=nz[2].spridx,x=2,y=12,mapid=np})end,function()return b"millandriver,?"end,function()n"12"local n=o(np).discvrdtiles add(n,"12|6")add(n,"12|7")add(n,"13|6")add(n,"13|7")end,function()return np=="home"end,function()n"10"end,function()return b"woods1,t"end,function()n"11"end,function()return d"12"end,function()l"?".intent="chase_candy_and_player"𝘦"k"end,function()local n=l"?"return h"mill,above,0"and n~=nil and n.mapid=="mill"end,function()𝘦"w"o"mill".playmapspr=124n"20"end,function()return h"woods1,left_of,1"end,function()n"14"end,function()return h"mill,left_of,7"end,function()n"13"end,function()return m(2,12,"mill")end,function()n6=2local e={}for n in all(n9)do if(n.spridx~=nz[2].spridx)add(e,n)
end n9=e n"17"end,function()return d"19"end,function()n6=1q{"w"}w"w,2|9"end,function()local n=l"?"return m(1,4,"mill")and n~=nil and n.mapid=="mill"end,function()x("millandriver",7,4)del(n𝘢,l"?")g("d","millandriver",5,4)g("t","millandriver",5,5)del(n𝘢,l"m")g("m","millandriver",6,5)n"22"n6=nil end,function()return np=="woods3"end,function()j"you completed act 1!"end,function()local n=o(np)return np=="woods3"and mget(n.cellx+nk,n.celly+t)==158end,function()n"23"n6=3end,function()local n=o"home"return np=="pottsfield"and s(nk,t,n.playmaplocx,n.playmaplocy)<3end,function()n"24"end,function()return b"barn,o"end,function()n"25"end,function()return b"barn,e"end,function()n"26"n6=nil w"e,7|13|7|5"end,function()return 𝘨("pottsfield",174)and 𝘺()end,function()𝘥"190"n𝘭+=1if(n𝘭<5)del(n𝘧,29)
𝘩()end,function()return 𝘨("pottsfield",158)and 𝘺()end,function()𝘥"190"n𝘮+=1if(n𝘮<5)del(n𝘧,30)
𝘩()end,function()return 𝘺()and np=="pottsfield"end,function()n"27"end,function()return n𝘺==4end,function()n6=nil a("thanks for grabbing me!","s")end,function()return h"school,below,13"end,function()n"29"q{"w","b"}𝘦"y"w"w,5|11,b,4|11"end,function()return l"y".mapid=="grounds"end,function()n"30"end,function()if(not 𝘢(34))return false
local n,e=p()return n==nil end,function()n𝘣+=1local n,e=0,0while(not z("grounds",n,e,nil))n,e=flr(rnd(16)),flr(rnd(16))
add(n9,{spridx=nz[5].spridx,x=n,y=e,mapid=np,cldwn=1})if(n𝘣<2)del(n𝘧,35)
end,function()local n,e=p()return n~=nil end,function()local n,e=p()if(n.cldwn==15)n.x,n.y=𝘫("grounds",n.x,n.y)n.cldwn=0
n.cldwn+=1del(n𝘧,36)end,function()local e,n=p()return n==0end,function()local e,o=p()n𝘵+=1if(n𝘵<2)a"haha yeah we got one!"del(n𝘧,37)else n"31"g("l",np,nk,t-1)l"l".intent="chase_candy_and_player"
del(n9,e)end,function()return n𝘵>1and np=="school"end,function()n"32"del(n𝘢,l"l")w"r,3|5,w,7|5,b,8|5,u,6|2,c,7|2"end,function()return 𝘢(38)and h"school,below,6"end,function()n"33"end,function()local n=l"r"return 𝘢(39)and s(nk,t,n.x,n.y)<=1end,function()n"34"end,function()return d"34"end,function()n"35"music(17)end,function()return d"35"end,function()n"36"add(n9,{spridx=126,x=6,y=2,mapid=np})add(n9,{spridx=126,x=7,y=2,mapid=np})end,function()return d"36"end,function()n"37"g("𝘤","school",7,6)w"𝘤,7|14,r,7|9"music(-1)for n in all(n9)do if(n.spridx==126)del(n9,n)
end end},split("|woods1,leave a trail of candy|woods1,give the turtle a candy|woods2,leave a trail of candy|woods2,inspect the strange tree|woods2,meet someone new|||woods3,search the bushes||millandriver,talk with the woodsman||millandriver,enter the mill|millandriver,find the frog!|pottsfield,visit the home|woods1,spot the turtle||millandriver,run back to your brother!|||mill,find a club|mill,use the club!|mill,jump the window to escape!,14||woods3,acquire new shoes||pottsfield,meet the residents|barn,meet the host|pottsfield,collect wheat,28|pottsfield,collect pumpkin,28||pottsfield,dig at the flower,31|school,start the lesson|grounds,go play outside,33|grounds,play 2 old cat,34|||school,run back to school!,37|school,cheer folks up,38|school,talk to the teacher,39|||","|"),{},{},{function()nl()end,function()na()end,function()ni()end,function()nd()end,function()nr()end,function()n0()end,function()n1()end,function()nf()end,function()ns()end,function()nc()end,function()nu()end,function()nh()end},"128#129#130#131#132#132,true#131,true#130,true#129,true#128,true#144#0#133#134#135#136#137#145#0#144,true#144#0#146#147#148#149#150#151#0#144,true#144#0#152#153#160#161#162#163#0#144,true#144#0#0#164#165#166#167#0#0#144,true#168#169#176#176#176#176#176#176#169,true#168,true",split"kitty,wirt,wirt jr.,george washington,mr. president,benjamin franklin,doctor cucumber,greg jr.,skipper,ronald,jason funderburker"function e(n,e)for n in all(n)do if(e==n)return true
end return false end function r(n,e,o)if(n)return e
return o end local n𝘸=(function()local o={}for n in all(split("g;greg;0;2;where is that frog o' mine!|wanna hear a rock fact?#w;wirt;1;4;uh, hi...|oh sorry, just thinking#b;beatrice;16;6;yes, i can talk...|lets get out of here!#k;;17;8;ribbit#a;the beast;32;34#m;the woodsman;33;36;i need more oil|beware these woods#?;the beast?;48;38;*glares at you*;2#d;dog;49;40;*barks*#t;black turtle;64;66;*stares blankly*#z;turkey;65;68;gobble. gobble. gobble.#o;pottsfield citizen 1;80;98;you're too early#i;pottsfield citizen 2;80;102;are you new here?#s;skeleton;81;70;thanks for digging me up!#p;partier;96;100;let's celebrate!#e;enoch;97;72;you don't look like you belong here;2#u;dog student;10;44;humph...|huh...#l;gorilla;113;12;*roaaar*!#j;jimmy brown;11;14#c;cat student;26;46;humph...|huh...#r;ms langtree;112;104;oh that jimmy brown|i miss him so...#n;the lantern;;76#𝘧;rock fact;;78#𝘦;edelwood;219;192#y;racoon student;27;194;humph...|huh...#𝘢;achievement get!;;196#𝘤;mr langtree;184;182;these poor animals","#"))do local n=split(n,";")local e=n[3]e=r(e=="",nil,tonum(e))add(o,{id=n[1],name=n[2],mapspridx=e,chrsprdailogueidx=tonum(n[4]),idle=r(#n>4,split(n[5],"|"),"huh?"),scaling=r(#n>5,tonum(n[6]),1)})end return o end)()function c(e)for n in all(n𝘸)do if(n.id==e)return n
end end local n𝘵,n𝘥=(function()local n=split("174,204,218,219,235,236,251#2,3,4,8,9,10,11#0,0,1,1,0,1,1","#")return{idxs=split(n[1]),clrmp_s=split(n[2]),clrmp_d=split(n[3])}end)(),(function()local o={}for n in all(split("exterior,woods1,somewhere in the unknown,0,16,0#exterior,woods2,somewhere in the unknown,0,0,14#interior,mill,the old grist mill,64,0,millandriver,226,7,2#exterior,millandriver,the mill and the river,16,0,0#exterior,woods3,somewhere in the unknown,32,0,0#interior,barn,harvest party,80,0,pottsfield,224,4,1#interior,home,pottsfield home,32,16,pottsfield,232,10,7#exterior,pottsfield,pottsfield,48,0,0#exterior,woods4,somewhere in the unknown,96,0,0#interior,school,schoolhouse,16,16,grounds,228,3,3#exterior,grounds,the schoolgrounds,112,0,0","#"))do local n=split(n)local e={type=n[1],id=n[2],title=n[3],cellx=n[4],celly=n[5]}if(n[1]=="interior")e.playmapid=n[6]e.playmapspr=n[7]e.playmaplocx=n[8]e.playmaplocy=n[9]
add(o,e)end return o end)()function o(e)for n in all(n𝘥)do if(n.id==e)return n
end end function 𝘬(e)for n=1,#n𝘥 do if(n𝘥[n].id==e)return n
end end n𝘢=(function()local o={}for n in all(split("t,13,7,woods1#m,6,7,woods2#o,6,12,barn#o,7,4,barn,loop,7|4|10|7#o,7,7,barn,loop,7|4|10|7#i,10,4,barn,loop,7|4|10|7#i,10,7,barn,loop,7|4|10|7#e,8,5,barn#r,8,9,school#u,7,11,school#c,9,11,school#y,9,13,school#z,7,6,home","#"))do local n=split(n)local e={charid=n[1],x=n[2],y=n[3],mapid=n[4]}if(#n>4)e.intent=n[5]e.intentdata=n[6]
add(o,e)end return o end)()local n𝘹,n𝘻=(function()local e={}for n in all(split("woods1;woods2;15,5|15,4;0,14|0,15#woods2;millandriver;14,0|15,0;0,13|0,14|0,15#millandriver;woods3;0,0|1,0;13,15|14,15|15,15#millandriver;mill;7,3;8,14#woods3;pottsfield;7,0|8,0;7,15|8,15#pottsfield;barn;4,2|5,2;7,13|8,13#pottsfield;woods4;9,0|10,0;6,15|7,15#woods4;grounds;7,0|8,0;6,15|7,15#grounds;school;3,4|4,4;7,14|8,14|11,1#pottsfield;home;10,8|11,8;7,12","#"))do local n=split(n,";")add(e,{mp_one=n[1],mp_two=n[2],mp_one_locs=split(n[3],"|"),mp_two_locs=split(n[4],"|")})end return e end)(),(function()local e,t={},1for n in all(split("k;led through the mist-k;by the milk light of moon-k;all that was lost is revealed-k;our long bygone burdens-k;mere echoes of the spring-k;but where have we come?-k;and where shall we end?-k;if dreams can't come true-k;then why not pretend?-k;how the gentle wind-k;beckons through the leaves-k;as autumn colors fall-k;dancing in a swirl-k;of golden memories-k;the loveliest lies of all+g;i sure do love my frog!-w;greg, please stop...-k;4;ribbit.-g;haha, yeah!+w;i dont like this at all-g;its a tree face!+w;is that some sort of deranged lunatic?-w;with an axe waiting for victims?-m;*swings axe and chops tree*;large-w;what is that strange tree?-g;we should ask him for help!-*;5+w;whoa... wait greg...;large-w;... where are we?;large-g;we're in the woods!;large-w;no, i mean;large-w;... where are we?!;large+w;we're really lost greg...-g;i can leave a candy trail from my pants!-g;candytrail. candytrail. candytrail!-*;2+b;help! help!-w;i think its coming from a bush?-*;9+b;help me!;large-g;wow, a talking bush!-b;i'm not a talking bush! i'm a bird!-b;and i'm stuck!-g;wow, a talking bird!-b;if you help me get unstuck, i'll-b;owe you one-g;ohhhh! you'll grant me a wish?!-b;no but i can take you to...-b;adalaid, the good woman of the woods!-g;*picks up beatrice out of bush*-w;uh uh! no!+m;these woods are a dangerous place-m;for two kids to be alone-w;we... we know, sir-g;yeah! i've been leaving a trail-g;of candy from my pants!-m;please come inside...;large-w;i don't like the look of this-k;ribbit.-g;haha, yeah!-*;13+w;oh! terribly sorry to have-w;disturbed you sir!-z;gobble. gobble. gobble.;large+g;wow, look at this turtle!-w;well thats strange-g;i bet he wants some candy!-t;*stares blankly*-*;3+?;*glares at you, panting*;large-g;you have beautiful eyes!-g;ahhhh!-*;18+w;wow this place is dingey-g;yeah! crazy axe person!-w;we should find a way to take him out-w;before he gets a chance to hurt us-g;i can handle it!-*;21+w;i dont think we should-w;go back the way we came+m;whats the rucus out here?-w;oh nothing sir!-g;nows my chance!+m;i work as a woodsman in these woods-n;keeping the light in this lantern lit;large-m;by processing oil of the edelwood trees-m;you boys are welcome to stay here-m;ill be in the workshop-g;okey dokey!+g;this bird art sculpture is perfect!-*;22+g;there! this little guy wanted a snack-t;*stares blankly*;large+m;ow! *falls onto ground*-g;haha yeah, i did it!-w;greg! what have you done!-w;hey greg... where did your frog go?-g;where is that frog o mine?-*;14+g;ahhh! the beast!-w;quick, greg, to the workshop!-w;we should be able to make it-w;out through a window out back!-?;*crashes through the wall*;large-*;23+g;we made it!-w;hopefully hes stuck there!-?;*gets stuck in the window*-?;*spits out a candy*-d;*looks at you happily*;large-g;look! hes my best friend now!+m;what have you boys done?!-m;the mill is destroyed-w;but we solved your beast problem!-m;you silly boys-m;that silly dog was not the beast-d;*bark! bark!*;large-m;he just swallowed that turtle-t;*stares blankly*;large-m;go now and continue your journey-w;we're sorry sir-m;beware the beast!+g;oh wow! i stepped on a pumpkin!-w;huh oh that's strange-w;i did too-g;haha i have a pumpkin shoe!-k;ribbit.+w;wow, greg, look!-w;a home! maybe they have a phone-b;oh this is just great-*;15+o;hey -o;who are you?;large-w;uh hello! we're just passing through-o;folks dont just pass through here-b;nope, i dont like this!-*;28+e;well, well, well,...-e;what do we have here?;large-b;nothing sir!-o;they tramped our crops!-i;look at their shoes!-e;for this you are sentenced-e;to...-e;a few hours of manual labour-w;oh uh okay?-e;come outside with me+e;you must collect 5 pumpkins-e;and 5 bundles of wheat-e;go now, and pay your dues!-*;29-*;30+e;now for your final act-e;you must dig a hole-e;6 feet deep, at the flower-e;take these shovels and-e;start digging-b;oh god!-*;32+r;and that children-r;is how you do addition-r;well hi there children!;large-r;please, take your seats-w;oh ok sure!-b;no wirt! ugh!-g;school? no way-g;lets go play outside-k;ribbit!-y;hmph!-*;34+g;i know, lets play 2 old cat-g;come on lets grab the cats!-k;ribbit.-y;hmph!-*;35+g;we did it!-g;isnt 2 old cat great?-l;*roaaar!*;large-g;aaaah! run!-y;*scatters*-*;38+g;phew, we made it!-r;how i do miss that-r;*jimmy brown*-r;time for lunch children;large-w;oh, ok!-b;ugh wirt please!-g;haha yay!+g;whats the matter rac?-y;*looks sad at food*;large-g;*takes a bite*-g;kinda bland-g;i got an idea!+g;here play like this!-g;*smashes piano keys*-r;like this?-g;good enough!+g;𝘰h, potatoes and molasses-g;𝘪f you want some, oh, just ask us-g;𝘵hey're warm and soft like puppies in socks-g;𝘧illed with cream and candy rocks+g;𝘰h, potatoes and molasses -g;𝘵hey're so much sweeter than 𝘢lgebra class-g;𝘪f you're stomach is grumblin'-g;𝘢nd your mouth starts mumblin'-g;𝘵here's only only one thing to -g;keep your brain from crumblin'-g;𝘰h, potatoes and molasses-g;𝘪f you can't see 'em put on your glasses-g;𝘵hey're shiny and large like a fisherman's barge-g;𝘺ou know you eat enough when you start seeing stars-g;𝘰h, potatoes and molasses-g;𝘪t's the only thing left on your task list-g;𝘵hey're short and stout,-g;𝘵hey'll make everyone shout-g;𝘧or potatoes and molasses-g;𝘧or potatoes and+𝘤;thats enough!;large-r;father!-𝘤;this is a school-𝘤;not a marching band;large-𝘤;give me those instruments-𝘤;goodbye!-r;oh father!","+"))do local n=split(n,"-")add(e,{})for n in all(n)do local n=split(n,";")local o=n[2]local l=tonum(o)if(l~=nil)o="ᶜ9["..split(n𝘯[l])[2].."]"
add(e[t],{speakerid=n[1],text=o,large=r(#n>2,n[3]=="large",false)})end t+=1end return e end)()function _init()assert(#n𝘯==ceil(#n𝘣/2))repeat local n=k(𝘭())if(not e(n𝘱,n))add(n𝘱,n)
until#n𝘱==4c"k".name=n𝘷[k(n𝘷)]end function _update()𝘮(n𝘤).update()end function _draw()𝘮(n𝘤).draw()end function ni()if(btn"5")n𝘤="mainmenu"
end function nd()cls()?"⁶-b⁶x8⁶y8       ᶜ4⁶.\0\0\0\0\0\0\0◝⁶.\0\0\0\0\0\0\0◝⁶-#⁶.\0\0\0\0\0\0\0ワ⁸⁶-#ᶜa⁶.\0\0\0\0\0\0\0⁸⁶-#ᶜ4⁶.\0\0\0\0\0\0\0³⁸⁶-#ᶜa⁶.\0\0\0\0\0\0\0ュ⁶-#ᶜ4⁶.\0\0\0\0\0\0\0「⁸⁶-#ᶜa⁶.\0\0\0\0\0\0\0フ⁶-#ᶜ4⁶.\0\0\0\0\0\0\0█⁸⁶-#ᶜa⁶.\0\0\0\0\0\0\0○⁶-#ᶜ4⁶.\0\0\0\0\0\0\0◝⁶-#⁶.\0\0\0\0\0\0\0ン⁸⁶-#ᶜa⁶.\0\0\0\0\0\0\0⁶⁶-#ᶜ4⁶.\0\0\0\0\0\0\0◆⁸⁶-#ᶜa⁶.\0\0\0\0\0\0\0p\n⁶-#   ᶜ4⁶.\0\0\0\0\0\0ナヲ⁶.\0\0\0ナヲュ◝◝⁶-#⁶.█◝◝◝◝◝○?⁸⁶-#ᶜa⁶.\0\0\0\0\0\0█ら⁶-#ᶜ4⁶.ン◝◝◝⁷³\0\0⁸⁶-#ᶜa⁶.\0\0\0\0ヲュ◝◝²4⁶.\0\0\0\0◝◝◝◝⁶.\0\0\0\0◝◝◝◝⁶.、~◝◝◝◝◝◝⁶.ヲ\0\0³◝◝◝◝⁶.フ\0\0\0◝◝◝◝⁶.◝ュナ\0⁷ᶠᶠ○⁶.\0¹¹⁷⁷⁷⁴\0⁶.²\0⁴⁴\0ᶜᶜᶜ⁶.ユナナナユヲヲス\n⁶-# ᶜ4⁶.\0\0\0\0ナュュヲ⁶.\0\0¹²¹ᵇ?か⁶-#⁶.ヲヲュ◜ᶜᵉ³¹⁸⁶-#ᶜa⁶.\0\0\0\0ユユュ◜²4⁶.\0\0\0\0゜゜◝◝⁶.らナナ@\0\0█ら⁶.◝◝◝◜◜◜◝◝²a      ⁶-#ᶜ4⁶.◝◝ヒろ▤0ユユ⁸⁶-#ᶜa⁶.\0\0¹³⁷ᶠᶠᶠ⁶-#ᶜ4⁶.リワ◝◝◝◜ュン⁸⁶-#ᶜa⁶.ᶜ⁸\0\0\0\0\0\0²4⁶.ヘきナ█ららナ\0\n⁶-#ᶜ4⁶.\0\0\0ら☉ヘムュ⁶.ヲ◜◝◝◝◝◝◝⁶-#⁶.ト○?゜゜ᶠ⁷⁷⁸⁶-#ᶜa⁶.\0█らナナユヲヲ²4⁶.◝◝◝ャリニ▒\0⁶.ユニト◝◝○◝◝⁶.ヤ◝◝◝◝◝◝◝²a ᶜ4⁶.\0\0\0\0ヲき#□⁶.\0\0\0\0\0⁴」c⁶.\0\0\0\0\0\0\0@⁶.\0\0\0\0\0\0■ ⁶.\0\0\0\0\0¹@\0⁶.\0\0\0\0\0¹¹\0⁶-#⁶.ナナナ  ナナナ⁸⁶-#ᶜa⁶.゜゜゜゜゜゜゜゜⁶-#ᶜ4⁶.ネフヤムっ••」⁶-#⁶.◝○◝◝◝◝◝よ⁸⁶-#ᶜa⁶.\0█\0\0\0\0\0\0\n⁶-#ᶜ4⁶.ュュンヌユユ…█²4ᶜa⁶.\0█\0\0\0\0\0\0⁶.ヲx○○~~~|⁶.²⁶テ◜◜◜ュヲ⁶.ヲ█¹³³³゜?⁶.◝◝>ゆ◜◜>◜⁶.?フヲ◝◝◝よ◜⁶.◝ョ◝◝◝◝フo⁶.○◝◝◝◝◝○ナ⁶.◜よ◝◝◝◝◝●⁶.◝◝◝◝◝◝◝○⁶.フフワ◝゜゜◝\0⁶.◝◝ヨネネフ◝\0⁶-#ᶜ4⁶.ナユpユナナナカ⁸⁶-#ᶜa⁶.゜ᶠᶠᶠ゜゜゜.⁶-#ᶜ4⁶.;9003???⁶.゜゜゜゜か♪😐を\n⁶.●ウフ◝◝◝◝◝⁶.◝◝◝◝◝◝よ゜⁶-#⁶.⬇️⬇️³³♥⁷⁷ᶠ⁸⁶-#ᶜa⁶.||||xヲヲユ⁶-#ᶜ4⁶.ᵇ	\0らるナチ█⁸⁶-#ᶜa⁶.ユユヲ<<、\0¹²4⁶.◝か@@オ▤スオ²a ᶜ4⁶.¹𝘢²²² ⁴⁴⁶.▮▮▮▮▮…▮▮⁶.▮ \0\0\0¹¹\0⁶.𝘢𝘢¹¹¹y𝘢▒⁶.\0\0\0\0\0\0█\0⁶.\0%!く! c\0⁶.²リ\"#b²ワ\0⁶.ナナ▒らユヲヤナ⁶-#⁶.○○○○○◝◝◝⁶.る𝘣²\0\0\0\0¹\n²4ᶜa⁶.\0\0\0\0らユx8⁶-#ᶜ4⁶.𝘰𝘰エホら█?○⁸⁶-#ᶜa⁶.\0\0\0⁶?○ら█⁶-#ᶜ4⁶.゜、8𝘱⬇️⁵⁴ᶜ⁸⁶-#ᶜa⁶.ナナら█\0²³³⁶-#ᶜ4⁶.らナっら\0¹ᶜヲ⁸⁶-#ᶜa⁶.¹³⁷?◝◜ユ\0²4⁶.ヲユッ◝◝゜'`⁶.◝◝◝◝◝トトヤ⁶.ャワワワ◝◝◝◝⁶.ヤヤ◝◝◝◝◝◝⁶.ヤ◝◝ヤ◝◝◝◝⁶.◜◜◝◝◝ヲリワ⁶.○◜◜ョャ⁷◝◝⁶.゜◝◝◝◝◝かよ⁶.□◝◝◝◝◝○◝⁶.●ヤ𝘨⁷⁶¹\0¹⁶-#ᶜ4⁶.◝◝◝○oᶠ゜゜⁶.¹¹¹¹\0\0\0\0\n²4ᶜa⁶.、゛ᵉᵉᵉᵉᵉᵉ⁶.\0\0\0\0\0\0ユユ⁶-#ᶜ4⁶.、==み3⁷\0、⁸⁶-#ᶜa⁶.³²🐱\0██エれ⁶-#ᶜ4⁶.ッンヲヲユユユナ⁸⁶-#ᶜa⁶.\0²⁷⁷ᶠᶠᶠ゜²4⁶.ラ◜ュてムムュュ⁶.ワンはヨヨヨルル⁶.◝ヤエ◜ュョョョ⁶.◝◝○o◝○○○⁶.◝トよよよャリフ⁶.ヤトトよよよ○○⁶.◝◝ト◝◝ヤ◝○⁶.◝◝トト◝ヤ◝ワ⁶.◝◝◝ャ◝◝ワ◝⁶.1z7??733⁶-#ᶜ4⁶.○゜??○○○○⁸⁶-#ᶜa⁶.\0ナらら████⁶-#ᶜ4⁶.\0█▒▒▒𝘢¹¹⁸⁶-#ᶜa⁶.\0¹\0\0\0\0\0\0\n⁶-#ᶜ4⁶.ニナるろ\0ᶜ8ま⁸⁶-#ᶜa⁶.゛゛<8ヲユら\0²4⁶.…███ユ◝゜\0⁶-#ᶜ4⁶.、<、\\░⁴9?⁸⁶-#ᶜa⁶.れ𝘤c#{³⁶\0⁶-#ᶜ4⁶.ネろらっ░☉█ュ⁸⁶-#ᶜa⁶.、880x\0\0\0²4⁶.ろ░ささ🅾️pxx⁶.ルレワワ◝³◝◜⁶.◝ョ◝◝◝ᵉ◝◝⁶.}○○○◝ ◝◝⁶.ヤトトよ○|◝◝⁶.◝◝◝◝◝◜◝◝⁶.v~◜ョョョョャ⁶.◝◝◝◜◜゛◜ョ⁶.ヒヤトト◝ᶜ◝○⁶.3#ckャ\0\0\0⁶-#ᶜ4⁶.○○ww0゜◝◝⁸⁶-#ᶜa⁶.██☉☉エ\0\0\0⁶-#ᶜ4⁶.\0@²⁶\0@⁷⁷⁸⁶-#ᶜa⁶.¹¹𝘢𝘢○\0\0\0\n⁶-#ᶜ4⁶.88880う>?⁶.ュュュ◜◝◝◝◝⁶.○◝◝◝◝◝◝◝⁶.ュュョ◝◝◝◝◝²4ᶜa⁶.ヲヲヲユららム◜⁶.ュね◝◝◝◝◝◝²a      ᶜ4⁶.██\0\0\0███²4  ⁶-#⁶.⁷⁷⁷ᶠᶠᶠᶠᶠ\n⁶.○◝◝◝◝◝◝◝²4   ᶜa⁶.◝◝◝◝◝◝◜◜²a ᶜ4⁶.\0\0\0\0\0\0⁸8     ⁶.████████²4  ⁶-#⁶.ᶠᶠ゜゜゜゜゜゜\n⁶.◝◝ンヲら\0\0\0⁶.◝◝◝◝◝◜◜◜²4  ᶜa⁶.◝◝◝◜◜◝◝◝²a ᶜ4⁶.8pユユユナナナ⁶.\0\0\0¹³³⁷³⁶.…` \0\0\0\0\0   ⁶.ナナナナナナナナ²4  ⁶-#⁶.゜゜゜゜????\n⁶.\0\0\0\0\0\0\0ᵉ⁶.◜◜◜◜◜◜◜◜²4  ᶜa⁶.◝◝◝◝◜◜◜◜²a ᶜ4⁶.ららナナナユユユ⁶.▒▒れれネ♥♥♥⁶.ᶠᶠ゜゜゜゜゜゜   ⁶.ナナナナナナナナ²4  ⁶-#⁶.○○○○○○◝◝\n⁶.エ◝◝◝◝◝◝◝²4   ᶜa⁶.◜◜ュユら███²a ᶜ4⁶.ユユユヲららら@⁶.⁷ᶠᶠ◆³¹³³⁶.゜??゜゜⁸\n゜  ⁶.\0\0\0\0█ららユ⁶.ナユヲ◝◝◝◝◝²4   \n    ᶜa⁶.█\0\0\0\0\0\0\0⁶.◝◝ユユ\0\0\0\0⁶.゜よ◝◝\0\0\0\0⁶.ュ◝◝◝◝\0\0\0⁶.◝◝◝◝◝\0\0\0⁶.◝◝◝◝◝\0\0\0⁶.◝◝◝◝゜\0\0\0⁶.³³³³\0\0\0\0  ⁶-#ᶜ4⁶.◝◝◝◝◝◝゜\0⁶.◝◝○゜ᶠ\0\0\0\n⁶.◝◝\0\0\0\0\0\0⁶.◝◝\0\0\0\0\0\0⁶.◝◝\0\0\0\0\0\0⁶.◝◝\0\0\0\0\0\0⁶.◝◝\0\0\0\0\0\0⁶.◝◝\0\0\0\0\0\0⁶.◝◝\0\0\0\0\0\0⁶.◝◝\0\0\0\0\0\0⁶.◝◝\0\0\0\0\0\0⁶.◝◝\0\0\0\0\0\0⁶.◝◝\0\0\0\0\0\0⁶.◝◝\0\0\0\0\0\0⁶.◝◝\0\0\0\0\0\0⁶.○○\0\0\0\0\0\0  "
?"	 ᶜ0press ❎ to\nenter the unknown",34,68
?"	 ᶜ9press ❎ to\nᶜ2enter the unknown",34,67
end function n1()if(btnp(4))n𝘤="mainmenu"
end function nr()if btnp(2)then t=t-1if(t==-1)t=2
sfx(0)elseif btnp(3)then t=t+1if(t==3)t=0
sfx(0)end if(btnp(4)or btnp(5))if t==0then n𝘤="intro"n"1"music(0)elseif t==1then n𝘤="controls"else stop()end
end function ns()if btnp(5)then local n=𝘤()[n_]if n~=nil then local e=#n.text if(n.time<e)n.time=e else n_+=1
end sfx(0)end if(n_>#n𝘻[1])n2()
end function z(n,e,o,l)for n in all(𝘧(n))do if(n.charid~=l and n.x==e and n.y==o)return false
end return v(e,o,n)and not(np==n and e==nk and o==t)end function 𝘫(a,e,o,i)local n={}for t=-1,1do for l=-1,1do if(z(a,e+t,o+l,i)and not(t+l==0))add(n,{e+t,o+l})
end end if(#n==0)return e,o
local n=n[k(n)]return n[1],n[2]end function g(n,e,o,t)add(n𝘢,{charid=n,mapid=e,x=o,y=t})end function nu()local i,f=#nx,false if btnp(5)and i>0then f=true sfx(0)local n=𝘤()[n_]if n~=nil and n.time~=nil then if(n.time<#n.text)n.time=#n.text else n_+=1
end if n_~=nil and n_>#𝘤()then if(type(nx[1])=="number")add(n𝘴,nx[1])
nx=𝘯(nx)n_=1end end if btn(5)then nj=nil for n=0,3do if(btn(n))nj=n
end else nj=nil end if(i>0)return
for n=1,#n𝘣,2do local o=ceil(n/2)if(not e(n𝘧,o)and n𝘣[n]())add(n𝘧,o)n𝘣[n+1]()f=true
end if nj==nil and i==0then local n=o(np)for n=0,3do if(btnp(n))n4(nk,t)break
end if btnp(2)and t>0and v(nk,t-1)then t=t-1elseif btnp(1)and nk<15and v(nk+1,t)then nk=nk+1n𝘬=false elseif btnp(3)and t<15and v(nk,t+1)then t=t+1elseif btnp(0)and nk>0and v(nk-1,t)then nk=nk-1n𝘬=true end end if(btnp(4)and i==0)𝘲()n𝘰={txt=c(nv).name,frmcnt=32}
local u,h=1for n in all(n𝘯)do local n=split(n)if(n[1]==np and not e(n𝘧,u)and 𝘢(r(#n>2,n[3],nil)))h=n[2]break
u+=1end local u,w,b=𝘳(np,nk,t)if u~=nil and not n𝘩 then n𝘩=true if 𝘬(u)<𝘬(np)then x(u,w,b)elseif h~=nil then if(i==0)a"we aren't done here yet... we should"a("ᶜ9["..h.."]")
else x(u,w,b)end elseif u==nil then n𝘩=false end if btnp(5)and n6~=nil and e(nz[n6].charids,nv)and not f then sfx(0)n𝘫=n6 if n6==1then add(n9,{spridx=nz[n6].spridx,x=nk,y=t,mapid=np})elseif n6==2then local e=l"m"if(s(nk,t,e.x,e.y)<2and not d"19")n"19"e.flipv=true
elseif n6==4then local n=o(np)local n,e=nk+n.cellx,t+n.celly if mget(n,e)==110then n𝘺+=1if(n𝘺==2)a"we're digging our own graves!"
if(n𝘺==4)mset(n,e,127)g("s",np,nk+1,t)t-=1n6=nil c"e".idle={"what a wonderful harvest"}
end end else n𝘫=nil end if i==0and not f then for n in all(𝘧(np))do for e=-1,1do for o=-1,1do if e~=o and n.x+e==nk and n.y+o==t then if(m(n.x,n.y,n.mapid))local e=c(n.charid).idle a(r(n.flipv or false,"owww...",e[k(e)]),n.charid)f=true
end end end end end for n in all(n𝘢)do 𝘴(n)end for n in all(n7)do 𝘴(n)end for n=-1,1do for l=-1,1do local n,t=nk+n,t+l if m(n,t,np)and not f then for l in all(split("122,123,138,139;what a nice old wagon#124,125,140,141;the poor old mill...#158;look at these pumpkins!#159;it says pottsfield ⬆️#174;looks like its harvest time!#204,236;its just a bush...#218,235,251;this tree sure is tall#220;a stump of some weird tree?#219;a creepy tree with a face on it#224,225,240,241;pottsfield old barn#226,227,242,243;the old grist mill#228,229,244,245;the animal schoolhouse#232,233,248,249;pottsfield old church#108;a rickety old fence#109;a scarecrow of sorts#110;the ground is higher here#127;a deep hole in the ground#42;what a nice view out this window#43;this is the door#58;its a large cabinet#59;its a comfortable chair#74;its a small desk#75;its a school desk#90;its a lounge chair#91;its a bundle of logs#106;its a ladder (i swear)#107;its a railing#143;its a piano!#177;its the mill's grinder!#178;its a jar of thick oil#179;its a broken jar of oil#200;its a chalk board#215;i found a rock fact!#216;its warm by the fireplace#217;a bundle of black oily sticks#180,181;a cafeteria bench and table#230,231,246,247;the town gazebo#232,233,248,249;pottsfield home","#"))do local d,l=split(l,";"),mget(n+o(np).cellx,t+o(np).celly)if e(split(d[1]),l)and i==0then a(d[2])f=true if(l==219)𝘱()
if l==215then if(not e(n𝘪,np))add(n𝘪,np)
a(split"put raisins in grape juice to get grapes!,dinosaurs had big ears but we forgot!,𝘪 was stolen from old lady daniels yard!"[#n𝘪],"𝘧",true)a(#n𝘪.." of 3 rock facts collected!","𝘧")mset(n+o(np).cellx,t+o(np).celly,202)if(#n𝘪==3)j"you found all 3 rock facts!"
end break end end end end end if(#nx>i or nj~=nil)sfx(2)
end function j(n)a("ᶜ9★ "..n.." ★","𝘢",true)if(not e(n𝘨,n))add(n𝘨,n)
end menuitem(1,"achievements",function()if(n𝘤~="playmap")return
for n in all(n𝘨)do j(n)end a(#n𝘨.." of 5 achievements gotten!")end)function 𝘢(n)return n==nil or e(n𝘧,n)or false end function 𝘱()if(not e(nq,np))add(nq,np)
a("*eerily howls in the cool fall wind*","𝘦",true)a(#nq.." of 6 edelwoods found!","𝘦")if(#nq==6)j"you found all 6 edelwoods!"
end function a(n,e,o)add(nx,{{speakerid=e or nv,text=n,large=o or false}})end function p()return 𝘶("grounds",nk,t,254)end function nw(e,o)for n in all(n𝘹)do if n.mp_one==e and n.mp_two==o then return split(n.mp_one_locs[1])elseif n.mp_one==o and n.mp_two==e then return split(n.mp_two_locs[1])end end end function 𝘳(e,o,t)for n in all(n𝘹)do if n.mp_one==e then for e in all(n.mp_one_locs)do local e,l=split(e),split(n.mp_two_locs[1])if(0+e[1]==o and 0+e[2]==t)return n.mp_two,l[1],l[2]
end elseif n.mp_two==e then for e in all(n.mp_two_locs)do local e,l=split(e),split(n.mp_one_locs[1])if(0+e[1]==o and 0+e[2]==t)return n.mp_one,l[1],l[2]
end end end return nil,nil,nil end function 𝘺()return l"e".mapid=="pottsfield"end function 𝘩()if(n𝘭>4and n𝘮>4)n"28"n6=4
end function 𝘴(n)if(n.cldwn==nil)n.cldwn=1
n.cldwn-=1if(n.cldwn==0)n.cldwn=16else return
local l=false for e in all(n7)do if(e.charid==n.charid)l=true
if(l and e.intent==nil and not z(e.mapid,e.x,e.y,e.charid))n.x,n.y=𝘫(e.mapid,e.x,e.y,e.charid)
end local e,a=n.intent,n.intentdata if e=="chase_candy_and_player"then local o=o(n.mapid)local o,l=o.cellx+n.x,o.celly+n.y local a=mget(o,l)if(a==191)mset(o,l,185)
if(a==142or a==175)mset(o,l,201)
if(a==178)mset(o,l,179)
e="walk"local e,o=𝘶(np,n.x,n.y,nz[1].spridx)local o=n.intentdata if n.mapid~=np then local n=nw(n.mapid,np)if(n~=nil and#n>1)o=n[1].."|"..n[2]
elseif e~=nil and s(n.x,n.y,e.x,e.y)<s(n.x,n.y,nk,t)then o=e.x.."|"..e.y else o=nk.."|"..t end n.intentdata=o end if e=="walk"then local a,e=n.mapid,split(n.intentdata,"|")local o,t=e[1]-n.x~=0,e[2]-n.y~=0if(o)n.x+=sgn(e[1]-n.x)
if(t)n.y+=sgn(e[2]-n.y)
if#e==2and not(o or t)then if n.intent=="walk"then n.intent=nil elseif n.intent=="chase_candy_and_player"then for e in all(n9)do if(e.mapid==n.mapid and e.spridx==nz[1].spridx and e.x==n.x and e.y==n.y)del(n9,e)
end end end local e,o,t=𝘳(a,n.x,n.y)if(not l and e~=nil)ng(n,e,o,t)
end if(e=="loop")local e=split(n.intentdata,"|")if n.x==e[3]and n.y~=e[2]then n.y-=1elseif n.y==e[2]and n.x~=e[1]then n.x-=1elseif n.x==e[1]and n.y~=e[4]then n.y+=1elseif n.y==e[4]then n.x+=1end
end function 𝘶(l,t,a,i)local o,e=nil,1000for n in all(n9)do local t=s(t,a,n.x,n.y)if(n.mapid==l and n.spridx==i and t<e)e=t o=n
end return o,e end function 𝘦(n)add(n7,l(n))for e in all(n𝘢)do if(e.charid==n)del(n𝘢,e)
end end function q(n)while(e(n,nv))𝘲()
for n in all(n)do add(n𝘢,l(n))end for o in all(n7)do if(e(n,o.charid))del(n7,o)
end end function 𝘲()add(n7,{charid=nv,x=nk,y=t,mapid=np})nv=n7[1].charid nk=n7[1].x t=n7[1].y n7=𝘯(n7)end local a,p,g,j=0,"pepjebs","studios",{}function nl()if(a%5==0and flr(a/5)<=#p)local n=flr(a/5)+1n3=sub(p,n,n)add(j,{letter=n3,x=10+12*n,y=45,age=0})
a+=1if(a>220)n𝘤="scene"music"23"
end function na()cls(7)if a==1then sfx(3)elseif a==135then sfx(4)end local n,e,o=58,72,1if a<60then o=7elseif a<120then o=6end print("⁶i"..g,n-#g,e,o)if(a>130and a<160)line(n-#g+(a-135),e-8,n-#g+(a-130)+4,e+16,7)line(n-#g+(a-135)+1,e-8,n-#g+(a-130)+5,e+16,7)
for e in all(j)do local n=e.age local o,t,l,a=flr(n/9)%7+8,3,e.x,false if n>100then a=true if(n>130and n<140)o=12else o=1
elseif n<100then t=8-flr(n/18)if n<85and n>75then n-=3l+=3elseif n>85and n<95then n+=3end end if(n>100)n=100
local n=e.y+(100-n)if(a)𝘷(e.letter,l+1,n+1,6,t)
𝘷(e.letter,l,n,o,t)if(e.age<200)e.age+=1
end end function nf()cls(0)print("ᶜa⬆️⬇️⬅️➡️ᶜ7 move",16,20)print("ᶜa❎ᶜ7 progress dialog or\n use item",16,40)print("ᶜa🅾️ᶜ7 switch characters",16,60)print("ᶜa❎+⬆️⬇️⬅️➡️ᶜ7 select object\n or npc",16,80)print("ᶜa🅾️ᶜ7 back to menu",8,118)end function n0()cls(0)local l,a,o=16,4,split(ee,"#")for n=1,6do for e=1,10do local o,t=split(o[(n-1)*10+e]),false if(#o>1)t=true
if(o[1]~=0)spr(o[1],l+e*8,a+n*8,1,1,t,false)else rectfill(l+e*8,a+n*8,l+e*8+8,a+n*8+8,o[1])
end end 𝘣("start",48,65,t==0)𝘣("controls",42,85,t==1)𝘣("quit",50,105,t==2)local n=65+t*20palt(5,true)pal(12,9)spr(234,24,n,1,1)spr(234,96,n,1,1,true,false)pal(12,12)palt(5,false)nm=𝘭()local n={{x=4,y=4},{x=108,y=4},{x=4,y=108},{x=108,y=108}}palt(0,false)for e=1,#n do f(n[e].x,n[e].y,17,17,2,10,9)spr(nm[n𝘱[e]].chrsprdailogueidx,n[e].x+1,n[e].y+1,2,2)end palt(0,true)end function nc()local n,e=32,16cls(0)f(n,e,64,52,0,4,5)pal(14,128,1)pal(5,133,1)pal(12,130,1)pal(1,132,1)pal(13,139,1)sspr(80,72,32,24,n,e+4,64,48)pal(14,14)pal(5,5)pal(12,12)pal(1,1)pal(13,13)𝘸()end function 𝘸()if#nx>0then local n=𝘤()[n_]if(n~=nil)n5(n)
end end function nh()local n=o(np)palt(0,false)palt(13,true)cls(139)map(n.cellx,n.celly)if(n𝘰~=nil and n𝘰.frmcnt>0and flr(n𝘰.frmcnt/5)%2==0)local n,e=nk*8,t*8pset(n,e+9,12)pset(n+7,e+9,12)line(n+1,e+8,n+6,e+8,12)line(n+1,e+10,n+6,e+10,12)pset(n,e+10,1)pset(n+7,e+10,1)line(n+1,e+11,n+6,e+11,1)
𝘰(0,c(nv).mapspridx,nk,t,1,n𝘬,false)for n in all(𝘧(np))do if n.x~=nil and n.y~=nil then local e,t=c(n.charid),1if(e.scaling~=nil)t=e.scaling
local o=false if(n.flipv)o=true
local l=false if(nk<n.x and not o)l=true
𝘰(0,e.mapspridx,n.x,n.y,t,l,o)end end for n in all(n9)do local e,o=n.x,n.y local t=r(z(n.mapid,e,o),0,-.5)if(n.mapid==np)𝘰(0,n.spridx,e+t,o,1,false,false)
end if(nj~=nil)local n=nj local o,n,e=𝘵(n)palt(5,true)spr(o,8*(nk+n),8*(t+e),1,1,n==-1,e==1)palt(5,false)
if n.type=="exterior"or n.id=="barn"then local d={}for l=0,15do for a=0,15do local i=false for n in all(𝘹(n7,{{x=nk,y=t}}))do if(s(l,a,n.x,n.y)<2.7)i=true
end local t=tostr(l).."|"..tostr(a)if(not n.discvrdtiles)n.discvrdtiles={}
if not i and not e(n.discvrdtiles,t)then add(d,{l,a})local n=mget(l+o(np).cellx,a+o(np).celly)if e(n𝘵.idxs,n)then for n=1,#n𝘵.clrmp_s do pal(n𝘵.clrmp_s[n],n𝘵.clrmp_d[n])end spr(n,8*l,8*a)for n=1,#n𝘵.clrmp_s do pal(n𝘵.clrmp_s[n],n𝘵.clrmp_s[n])end else if(#n𝘦==0and flr(rnd(30000))==0)add(n𝘦,{frmcnt=35,type="eyes",x=l,y=a})
rectfill(8*l,8*a,8*l+7,8*a+7,0)end elseif not e(n.discvrdtiles,t)then add(n.discvrdtiles,t)end end end end for o in all(n𝘦)do local t=tostr(o.x).."|"..tostr(o.y)if o.frmcnt>0and not e(n.discvrdtiles,t)then if o.type=="eyes"then local t,n,e=7,o.x*8,o.y*8if(o.frmcnt<8or o.frmcnt>27)t=1else pset(n+1,e+4,6)pset(n+3,e+4,6)pset(n+2,e+3,6)pset(n+2,e+5,6)pset(n+4,e+4,6)pset(n+6,e+4,6)pset(n+5,e+3,6)pset(n+5,e+5,6)
pset(n+2,e+4,t)pset(n+5,e+4,t)end o.frmcnt-=1else del(n𝘦,o)end end local e,n=n6~=nil and e(nz[n6].charids,nv),nil if n𝘰~=nil and n𝘰.frmcnt>0then n=n𝘰.txt n𝘰.frmcnt-=1else end 𝘻(1,c(nv).mapspridx,n)if(e)local e=nz[n6]𝘻(114,e.spridx,r(n~=nil,e.name,nil))
i=n𝘳 if(i~=nil and i.frmcnt>0)f(i.x,i.y,#i.txt*4+4,8,4,10,9)u(i.txt,i.x+3,i.y+3,2)u(i.txt,i.x+2,i.y+2,9)i.frmcnt=i.frmcnt-1
palt(13,false)𝘸()end function 𝘻(e,l,o)local n,t=1,s(nk,t,1,e/8)<3if(t)n=114
if o~=nil then if(t)n=90
f(n,e,#o*4+12,11,4,10,9)u(o,n+12,e+5,2)u(o,n+11,e+4,9)else f(n,e,11,11,4,10,9)end spr(l,n+2,e+2)end function 𝘰(r,i,o,t,n,l,a,e)e=e or 8local i,d=i%16*8,i\16*8for n=0,15do pal(n,r)end n=n or 1n*=e sspr(i,d,e,e,o*8,t*8+1,n,n,l,a)sspr(i,d,e,e,o*8+1,t*8,n,n,l,a)sspr(i,d,e,e,o*8+1,t*8+1,n,n,l,a)for n=0,15do pal(n,n)end sspr(i,d,e,e,o*8,t*8,n,n,l,a)end function l(e)for n in all(nn())do if(n.charid==e)return n
end end function 𝘤()local n=nx[1]if(type(n)=="number")n=n𝘻[n]
return n end function h(n)local n=split(n)if(n[1]~=np)return false
for l=2,#n,2do local e,o=n[l],tonum(n[l+1])if e=="on"then local n=tonum(n[l+2])if(nk~=o or t~=n)return false
l+=1end if(e=="above"and t<=o or e=="right_of"and nk<=o or e=="below"and t>=o or e=="left_of"and nk>=o)return false
end return true end function m(n,e,o)local l,a,i=𝘵(nj)return l~=nil and o==np and nk+a==n and t+i==e end function 𝘨(e,l)local a,i,d=𝘵(nj)local n=o(np)return e==np and a~=nil and mget(n.cellx+nk+i,n.celly+t+d)==l end function 𝘥(e)local n,l,a=𝘵(nj)local n=o(np)mset(n.cellx+nk+l,n.celly+t+a,e)end function 𝘪(e,o,n,l)return n𝘫==e and np==o and(n==nil or n==nk and l==t)end function b(n)local t=split(n)if(np~=t[1])return false
for n in all(𝘧(np))do local l=n.x.."|"..n.y if(e(o(np).discvrdtiles,l)and n.charid==t[2])return true
end return false end function d(n)return e(n𝘴,tonum(n))end function n(n)add(nx,tonum(n))end function n4(e,o)for n in all(n7)do if(flr(rnd(2))==0and n.intent==nil)ne(n,e.."|"..o)
end end function w(n)local n=split(n)for e=1,#n,2do ne(l(n[e]),n[e+1])end end function ne(n,e)n.intent="walk"n.intentdata=e end function n2()music(-1)n𝘤="playmap"nv="g"n_=1nx={}x("grounds",8,8)n7={{charid="w",mapid="grounds",x=nk,y=t},{charid="k",mapid="grounds",x=nk,y=t},{charid="b",mapid="grounds",x=nk,y=t}}pal(14,14,1)pal(5,5,1)pal(12,12,1)pal(1,1,1)pal(13,13,1)end function ng(n,e,o,t)n.x=o n.y=t n.mapid=e local e=split(n.intentdata,"|")if(n.intent=="walk"and#e>2)n.intentdata=e[3].."|"..e[4]
end function v(t,l,n)local n=o(n or np)local o=mget(t+n.cellx,l+n.celly)return e(en,o)and(n.type~="interior"or not e({207,223,239},o))end function x(n,l,a)np=n nk=l t=a for n in all(n7)do n.mapid=np n.x=nk n.y=t n.intent=nil n.intentdata=nil end local n=o(np)if(n.type=="interior")t-=1
local o=63-2*#n.title n𝘳={x=o,y=12,txt=n.title,frmcnt=45}local o,t=n.cellx,n.celly if not e(n𝘲,np)then local i=n8()for n=0,15do for l=0,15do local a=mget(n+o,l+t)if(e(i,a))local e=nb(a)local a=k(e)mset(n+o,l+t,e[a])
end end add(n𝘲,np)end for n in all(n𝘥)do if(n.type=="interior"and n.playmapid==np)local n,e,o=n.playmaplocx+o,n.playmaplocy+t,n.playmapspr mset(n,e,o)mset(n+1,e,o+1)mset(n,e+1,o+16)mset(n+1,e+1,o+17)
end end function s(n,e,o,t)return sqrt((o-n)^2+(t-e)^2)end function 𝘮(o)local e=split"boot,scene,mainmenu,controls,intro,playmap"for n=1,#e do if(e[n]==o)return{update=n𝘶[n*2-1],draw=n𝘶[n*2]}
end end function k(n)return flr(rnd(#n))+1end function 𝘭()no={}for n in all(n𝘸)do if(not e({-1,196},n.chrsprdailogueidx))add(no,n)
end return no end function 𝘣(n,e,o,l)f(e,o,#n*4+8,12,4,10,9)local t=0if(l)t=10u(n,e+5,o+5,2)
u(n,e+4,o+4,t)end function nn()return 𝘹(n7,n𝘢)end function 𝘹(e,o)local n={}for e in all(e)do add(n,e)end for e in all(o)do add(n,e)end return n end function n5(n)if(n.time==nil)n.time=1
f(8,100,112,24,4,10,9)local o=n.speakerid local l=c(r(o=="*",nv,o))local o,t,a=l.name,n.text,n.time print(o,30,104,2)print(o,29,103,9)local o=t if(a<#t)o=sub(t,1,a)n.time+=1
if#o<=22then u(o,29,110,0)else local n=-1for t=23,1,-1do if(e(split(",.? ",""),sub(o,t,t)))n=t break
end u(sub(o,1,n).."\n"..sub(o,n+1),29,110,0)end f(10,103,17,17,1,6,5)palt(13,true)spr(l.chrsprdailogueidx,11,104,2,2)if(n.large)f(30,22,68,68,1,6,5)𝘰(0,l.chrsprdailogueidx,4,3,4,false,false,16)palt(13,false)
local n,e=117,2if(#t==#o)n,e=117,10
if(btn(5))n,e=118,9
if(n==117)print("❎",112,118,0)
print("❎",112,n,e)end function 𝘵(n)if(n==nil)return nil
local e,o,t=234,0,2*n-5if(n==2or n==3)e=250else o=2*n-1t=0
return e,o,t end function 𝘯(n,e)nt={}for e=(e or 1)+1,#n do add(nt,n[e])end return nt end function 𝘧(o)local n={}for e in all(nn())do if(e.mapid==o)add(n,e)
end return n end function f(o,t,n,e,r,i,d)local l,a,e,n=o+1,t+1,o+n,t+e rectfill(l,a,e-1,n-1,r)line(l,t,e-1,t,i)line(l,n,e-1,n,d)line(o,a,o,n-1,i)line(e,a,e,n-1,d)pset(o,n,0)pset(e,n,0)line(l,n+1,e-1,n+1,0)end local n=split("206;206,221,237#238;222,238#207;207,223,239#235;235,251,218#205;202,203,205#236;204,236","#")function nb(e)for n in all(n)do local n=split(n,";")if(e==n[1])return split(n[2])
end return nil end function n8()local e={}for n in all(n)do local n=split(n,";")[1]add(e,n)end return e end function u(n,...)print(ny("^"..n),...)end function ny(n)local e,l,o,t="",false,false for a=1,#n do local n=sub(n,a,a)if n=="^"then if(o)e=e..n
o=not o elseif n=="~"then if(t)e=e..n
t,l=not t,not l else if o==l and n>="a"and n<="z"then for e=1,26do if(n==sub("abcdefghijklmnopqrstuvwxyz",e,e))n=sub("𝘢𝘣𝘤𝘥𝘦𝘧𝘨𝘩𝘪𝘫𝘬𝘭𝘮𝘯𝘰𝘱𝘲𝘳𝘴𝘵𝘶𝘷𝘸𝘹𝘺𝘻[\\",e,e)break
end end e=e..n o,t=false,false end end return e end function y(e,o)for n=0,319,4do poke4(e+n,peek4(o+n))end end function 𝘷(o,t,l,n,e)poke(17792,peek(24320+n))poke2(17793,peek2(24320))poke4(17795,peek4(24360))poke2(17799,peek2(24369))poke(17801,peek(24371))poke(24320+n,n)poke2(24320,n==0and 4352or 272)y(17472,0)y(0,24576)camera()fillp(0)rectfill(0,0,127,4,(16-peek(24320))*0x.1)print(o,0,0,n)y(17152,24576)y(24576,0)y(0,17152)camera(peek2(17795),peek2(17797))sspr(0,0,128,5,t,l,128*e,5*e)y(0,17472)poke(24320+n,peek(17792))poke2(24320,peek2(17793))fillp(peek2(17799)+peek(17801)*.5)end
__gfx__

__map__

__sfx__

__music__
00 05404040
00 06404040
00 07404040
00 08404040
00 09404040
00 0a404040
00 0b404040
00 0c404040
00 0d404040
00 0e404040
00 0f404040
00 10404040
00 11404040
00 12404040
00 13404040
00 14404040
04 15164040
00 17404040
00 18404040
00 19404040
00 1a404040
00 1b1c4040
04 1d404040
00 1e1f2021
00 22232425
00 26272829
00 2a2b2c40
00 2d2e2f30
00 31323334
00 35363738
00 393a3b3c
0c 3d3e3f00
0f 00000000
07 00000040
06 40004040
__label__

__meta:title__
over the garden wall
made by pepperoni-jabroni